...
![建築パネル「製作とユーティリティ」（カテゴリ全体表示）](../images/build-panel-utility.jpg)

Windroseの建築システムでは、素材を使って自由に拠点を建設できます。簡易シェルターから豪邸・要塞まで、複数の建築スタイルが用意されています。

建築パネルは左サイドにカテゴリ（製作とユーティリティ／寝具とベッド／床と壁／屋根／柱と梁／階段と橋／窓と扉／装飾／農業／軽量構造物）が並び、選択したカテゴリのアイテムが中央グリッドに表示される。アイテムを選ぶと右側に詳細・必要素材が出る。

![建築パネル：作業台選択時（必要素材：木材）](../images/build-panel-workbench.jpg)
