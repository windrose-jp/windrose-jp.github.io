...
- [攻略] **新パッチ 0.10.0.5.120-073042fb（2026-05-04）を patch-notes.md に追記**：セーブファイル保管・バックアップシステムの全面刷新（10分間隔の自動バックアップ + 終了時バックアップ、旧フォーマットからの自動マイグレーション、マイグレーション不能な破損セーブを通知する復旧 UI）、クラフトステーションで D-pad 左が「Take All」に誤マッピングされていた不具合の修正、既知の問題（0.10.0.4 以前作成キャラの削除不可・設定リセット）を一次情報（Steam News）から要約
- [攻略] **troubleshooting.md の Fatal Error セクションに 0.10.0.5.120 の復旧 UI を追記**：セーブ破損時の挙動が「マイグレーション不能ファイルを通知ウィンドウで提示する」仕様に変わった点を明示し、パッチノート該当節へリンク
