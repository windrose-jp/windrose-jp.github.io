# 更新履歴

## 全更新履歴

<!-- FULL_CHANGELOG_START -->
<!-- FULL_CHANGELOG_END -->

---

- 2026-04-21: [攻略] ゲーム概要の販売数（50万本→100万本）・同時接続数（113K→220K超）・配信プラットフォーム（Epic/Stove追加）を更新／防具セット全種の2P/4Pボーナスを実数値に更新（Conquistador 2P=DR+15%・Pikeman 2P=HP+160/4P=2H+15%・Privateer 2P=Crit+10%/4P=CritDmg+10%per敵・Tracker 2P=回復+35%/4P=DR+15%・Flibustier 2P=スタミナ-20%・Marksman 2P=移動スタミナ-30%）／Naval Tactics全5種に更新（Ambush・No Quarter・Stretch The Supply を追加）／大砲Epic派生3種（Devastating・Perfectly Ordered・Tempered）の効果を追加／Drake's Double-Barreled Pistolの脆弱性デバフ効果を追記／タレントのランク2/3値を追記（Marathon Runner・Just a Flesh Wound・Stout Frame）
- 2026-04-20: [攻略] タレント全38個の効果を正確な数値に更新（FextraLife検証）／Marathon Runnerの効果を「スタミナ消費削減」から「最大スタミナ+20」に訂正
- 2026-04-19: [攻略] 海戦ガイドに速度管理詳細（3/4速・1/4速の使い分け）・砲撃照準tips・Combat Repair Kitレシピ・Tortuga派閥ベンダー一覧（Smugglers Naval Tactics書）を追記／船の種類にKetch取得クエスト条件（7人救出・素材）・Frigate価格（3,000P）・36-Pounderダメージ（2,500/15秒）・沈没時インベントリ仕様を追記／船カスタマイズページを実内容で整備（Wharfカスタマイズタブ・Hull Bracing強化素材）
- 2026-04-18: [攻略] Hotfix 0.10.0.2.54 パッチノート追加（Boar Whistle修正・アニメーション修正・クラウドセーブ告知を追記）／マルチプレイページ更新（専用サーバー・招待コード・8人上限計画）／序盤イノシシ注意Tip追加／ゲーム概要にローンチ数値（50万本・113K同時・88%好評・正式リリース目標年）を追加／武器数を77種に修正・防具ページにアクセサリ節を追加／船種ページにバリエーション（Stock/Brethren/Blackbeard）を追記／ロードマップを大幅拡充（Ashlands・DLSS/FSR・ランダムルート等）／Tips: ミッションエリアの敵船密集に関する注意を追加
- 2026-04-14: サイト初期構築
