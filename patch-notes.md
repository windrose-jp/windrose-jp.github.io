...
### Patch 0.10.0.5.120-073042fb（2026年5月4日）

**セーブファイル保管・バックアップシステムの全面刷新**を主目的とした緊急アップデート。4月から続いていたクラウドセーブ破損問題への根本対策として配信。

**セーブシステム刷新:**
- セーブファイル用の**新しい保管・バックアップシステム**を実装
- **10分間隔のゲームプレイ中バックアップ + ゲーム終了時バックアップ**を自動実行（従来の「起動ごとに 1 つ・最大 30 件」から仕様変更）
- 旧フォーマット（〜0.10.0.4 系）から新フォーマットへの**自動マイグレーション機構**を追加。多くの場合シームレスに完了
- マイグレーション不能な破損セーブを検出した場合、**該当ファイルを通知ウィンドウで提示し削除を促す UI** を表示

**バグ修正:**
- クラフトステーションUIで **D-pad 左** が誤って「Take All（一括取得）」にマップされていた不具合を修正

**既知の問題（公式表記）:**
- **Version 0.10.0.4 以前に作成したキャラクターは削除不可**（Steam Cloud 同期の都合。次回パッチで対応予定）
- 本パッチ適用後、**ゲーム設定が初期化される**。必要に応じて手動でバックアップを推奨

> 情報源: [Steam ニュース 0.10.0.5.120 パッチノート](https://store.steampowered.com/news/app/3041230/view/658231077844812605) / [patchbot.io/games/windrose](https://patchbot.io/games/windrose) / [Escapist Magazine](https://www.escapistmagazine.com/news-windrose-patch-fixes-corrupted-save-files/)
