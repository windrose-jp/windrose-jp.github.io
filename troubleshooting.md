...
## ⚠️ 【進行中】Tortuga ファストトラベル時のフリーズ・クラッシュ

> 📢 **直近報告**: Tortuga（トルトゥーガ）へファストトラベル/テレポートした際に、ゲームが数秒〜無限にフリーズする、またはブラックスクリーンでクラッシュする症状が複数プレイヤーから報告されています。Patch 0.10.0.6（2026-05-17〜18）配信後も解消されておらず、Steam Community でスレッドが継続中。

### 報告されている症状

| 症状 | 内容 |
|------|------|
| **Tortuga 到着時のフリーズ** | ファストトラベル後にロードが進まず、タスクマネージャ強制終了が必要な例あり |
| **ブラックスクリーン → 無応答** | ファストトラベル直後に画面が真っ暗のまま戻らない |
| **Ketch（船）で行くと多発** | Ketch でのファストトラベルは特に発生率が高いとの報告 |
| **他のメンバーが Tortuga にテレポートした際の連鎖フリーズ** | マルチプレイで一時的に数秒の遅延が出る報告 |

### コミュニティで共有されている回避策（一時的）

> ⚠️ **未公式の回避策**です。複数スレッドで「これで通った」と報告されているもののみ列挙します。

- **シェーダー品質を High に設定**（Medium だとクラッシュが多いとの複数報告）。タイトル画面のグラフィック設定で変更
- **Tortuga へは Bell（鐘）でファストトラベル**する。Ketch（船）でのファストトラベルより安定との報告
- 多数キャラ運用時は**別キャラで一度ワールドへ入り直してから本キャラに戻る**ことで一時的に解消する例あり

### 公式対応状況

- 公式の修正告知は現時点（2026-06-02）でなし。Patch 0.10.0.6（2026-05-17〜18）以降、新規パッチは未配信
- Patch 0.10.0.6 では「テレポーター」「ファストトラベル」の直接的な修正は含まれていない
- 影響を受けた場合は [windrose.support](https://windrose.support/) からのバグ報告を推奨

→ 議論スレッド例:
- [Steam Community: Fast Travel Crash Bug ?](https://steamcommunity.com/app/3041230/discussions/0/837249693974041219/)
- [Steam Community: Game crash when teleporting to Tortuga](https://steamcommunity.com/app/3041230/discussions/0/837249543665996675/)
- [Steam Community: Game sometimes freezes when fast traveling to Tortuga](https://steamcommunity.com/app/3041230/discussions/0/807975352045511352/)
- [Steam Community: Fast Travel to Tortuga Freeze](https://steamcommunity.com/app/3041230/discussions/0/833873371400834658/)
...
- **2026-05-04 のパッチ 0.10.0.5.120-073042fb** でセーブ保管・バックアップシステムが刷新された：10分間隔の自動バックアップ + 終了時バックアップ、旧フォーマットからの自動マイグレーション、マイグレーション不能な破損ファイルを通知ウィンドウで提示する復旧 UI を内蔵。詳細は [パッチノート](patch-notes.md#patch-01005120-073042fb2026年5月4日) を参照
