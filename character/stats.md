...
**Steam 実プレイヤー検証**: Vitality は約 +13.3 HP/pt で **30 まで一定**、31 以降に逓減（約 +8 HP/pt）するとの報告。一部の英語攻略サイトに見られる「**20 でソフトキャップ**」という記述は実プレイヤー検証と矛盾しており、裏付けが取れていない記述と判断（[Steam: Level up Vitality is very nonsensical](https://steamcommunity.com/app/3041230/discussions/0/802345327968516331/) 参照）。
...
> ⚠️ **注意**: 一部の英語攻略サイトには「encumbrance system」「重量超過でスタミナ消費増」などの記述が見られますが、これらは**裏付けの取れていない情報**です。Steam 公式コミュニティでは「重量システムは存在しない」が一貫した結論となっています。本 wiki は実プレイ・一次情報に従って訂正済みです（2026-04-29）。
