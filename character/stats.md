...
**Steam 実プレイヤー検証**: Vitality は約 +13.3 HP/pt で **30 まで一定**、31 以降に逓減（約 +8 HP/pt）するとの報告。BisectHosting 等の英語攻略サイトに見られる「**20 でソフトキャップ**」という記述は実プレイヤー検証と矛盾しており、AI 生成記事に伝播した誤情報の可能性が高い（[Steam: Level up Vitality is very nonsensical](https://steamcommunity.com/app/3041230/discussions/0/802345327968516331/) 参照）。

| Vitality | HP/point の傾向 |
|----------|----------------|
| 〜30 | 約 +13/pt（一定） |
| 31〜 | 約 +8/pt（逓減） |

> 出典: Steam Community 実プレイヤー検証（個別計測値の範囲）。**公式仕様としての確定値ではないため、本格的なビルド構築時はゲーム内で実測することを推奨**。

### ⚠️ 遠距離武器のスタミナ消費（実機検証推奨）

**Pistol / Musket / Blunderbuss などの遠距離武器は射撃時に Stamina を**「ほぼ消費しない、または 0 とされる」**との記述が複数の英語攻略サイト（method.gg / allthings.how 等）で見られる**。Steam 公式コミュニティ・Reddit では具体的な数値検証が確認できないため、「完全に 0」かどうかは未検証。Marksman ビルドの構造（Endurance 0 / Precision・Vitality 全振り）が成立する状況証拠はあるため、**機能的に低消費なのは事実**だが、正確な数値は実機で確認推奨。

### 片手武器+ピストルの構造的優位

- **片手武器は off-hand にピストル（Pistol）を装備可能**（両手武器は不可）
- 戦闘中の射撃→近接持ち替えがシームレス
- **Drake's Pistol** で Vulnerability を付与してから近接で叩く戦術が強力（[Plague Mark コンボ](builds.md#plague-mark-コンボs-火力ループ)参照）

## 重量システムは存在しない

**Windrose に重量（Weight / Encumbrance）システムは実装されていません**。アイテムに重量パラメータは設定されておらず、積載重量による移動速度低下・スプリント不可などのペナルティは発生しません。

> 出典: [Steam Community: there are no weight limits](https://steamcommunity.com/app/3041230/discussions/0/807975131067662423/)（複数の実プレイヤー発言で確認済み）

代わりに**インベントリスロット数による制限**のみが存在します（下記参照）。

> ⚠️ **注意**: mobalytics.gg / xmodhub / neonlightsmedia 等の一部英語攻略サイトには「encumbrance system」「重量超過でスタミナ消費増」などの記述が見られますが、これらは **AI 生成と推定される誤情報**です。Steam 公式コミュニティでは「重量システムは存在しない」が一貫した結論となっています。本 wiki は実プレイ・一次情報に従って訂正済みです（2026-04-29）。
