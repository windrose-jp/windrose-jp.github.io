...
![海戦砲撃](../images/naval-battle-day.jpg)
...
![敵船炎上](../images/naval-battle-fire.jpg)
...
![乗り込み戦（甲板）](../images/naval-boarding-deck.jpg)
...
- **ラッパ銃（Blunderbuss）/ マスケット銃（Musket）で壁際から射撃**する戦法が推奨
- 近接戦に引きずり込まれると全方位から被弾する

![敵船内部](../images/naval-boarding-interior.jpg)
...
![夜間航行](../images/naval-deck-night.jpg)

![舵輪での操舵（夜間・未支配の海域）](../images/naval-helm-night.jpg)

![フリゲート甲板からの航行（昼間・「黒ひげの海域」入域時）](../images/ship-deck-blackbeard-sea.jpg)

> 操舵中は画面下部に **コンパス（風向と船首方向）** と **スロットル段階** が表示される。右上の地域名が「未支配の海域（Lawless Waters）」になっている海域では海賊の襲撃確率が上がる。「黒ひげの海域」など派閥領海に入ると右上に地域名が表示され、その派閥船との遭遇率が変動する。
...
→ 詳細は専用ページ [クルー雇用](crew-hiring.md) を参照。要点:

| 項目 | 仕様 |
|------|------|
| 雇用場所 | Tortuga |
| 支払い | **Piastres** |
| 種類 | Base Manager / Farming / Mining / **Ship Crew（ボーディング戦闘参加）** |
| ベース効果 | 拠点ステーション稼働率 +30% など職種別 |
| Ship Crew | ボーディング戦闘に参加 / Deckhand は船内ハンモックで睡眠（昼夜ルーチン） |
...
| **Brethren of the Coast** | Brigantine・Frigate の設計図（名声条件あり） |
