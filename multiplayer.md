...
| **敵スケーリング** | 人数連動でHP・ダメージ補正。**サーバー上の全アクティブ人数で算出される（地理的距離は無関係）**ため、フレンドが大陸の反対側にいても自分の周辺敵が硬くなる。ソロ進行したい場合は他メンバーをログアウトさせるかワールド設定の「EDIT」で難易度を下げる。**3人以上ボスHPがバグで過大化していた問題は Hotfix 0.10.0.2.54 で修正済み** |
| **資源リスポーン周期** | カスタム可 |
